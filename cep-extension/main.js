...
// Compatibility: try multiple require paths for different CEP versions
var nodeRequire = (typeof cep_node !== "undefined" && cep_node.require) ? cep_node.require
...
try {
  ...
  // fallback: try window.cep_node if available
  try {
    fs = window.cep_node.require("fs");
    path = window.cep_node.require("path");
    os = window.cep_node.require("os");
  } catch (e2) {
    console.error("Failed to load Node.js modules:", e2);
  }
}

if (!fs) {
  log("FATAL: Node.js fs module not available. Check --enable-nodejs in manifest.", "err");
}

tempDir = (path && os) ? path.join(os.tmpdir(), "premiere-mcp-bridge") : "";

function ensureDir(dir) {
  if (!fs) { log("Error creating dir: fs module not loaded", "err"); return; }
  try {
...
(function init() {
  ...
  if (fs && tempDir && fs.existsSync(tempDir)) {
