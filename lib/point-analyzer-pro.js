...
function estimatePointCount(captions) {
  ...
  // 점수대별 후보 개수
  const strong = scored.filter(s => s >= 20).length;   // 확실한 포인트 (질문/강조/결론 등)
  const medium = scored.filter(s => s >= 13 && s < 20).length; // 그럭저럭 쓸만한

  // 영상 길이 (분)
  const totalSec = merged.length > 0 ? merged[merged.length - 1].end : 0;
  const minutes = totalSec / 60;

  // 내용 기반 추정: 확실한 거 전부 + 그럭저럭한 거 절반
  const contentBased = strong + Math.round(medium * 0.5);

  // 길이 기반 상한 (분당 ~6개가 이 채널 스타일, 너무 빽빽하지 않게)
  const lengthCap = Math.max(8, Math.round(minutes * 6));

  // 둘 중 작은 값, 그리고 합리적 범위로 클램프 (최소 8, 최대 40)
  let suggested = Math.min(contentBased, lengthCap);
  suggested = Math.max(8, Math.min(40, suggested));

  return suggested;
}

module.exports = { extractPoints, scoreText, estimatePointCount, CATEGORY_KEYWORDS };
