...
function scoreText(text, position, totalCount) {
  ...
  // 내용 점수가 거의 없으면(키워드/패턴 매칭 0) 위치/길이 보너스 줘봤자 필러
  // → 내용 신호가 있을 때만 보너스 적용 (위치/길이는 타이브레이커 역할)
  const hasContentSignal = maxCatScore >= 10;

  // 4. 위치 가중치 (대폭 축소 - 내용 있을 때만)
  const positionRatio = position / Math.max(1, totalCount - 1);
  if (hasContentSignal) {
    if (positionRatio < 0.05) score += 4;        // 인트로 (작게)
    if (positionRatio > 0.92) {
      score += 4;                                // 아웃트로 (작게)
      if (category === 'emphasis' && score > 25) category = 'conclusion';
    }
  }

  // 5. 길이 보너스 (대폭 축소 - 타이브레이커 수준)
  if (t.length >= 15 && t.length <= 40) score += 3;

  // 6. 특수 문자 보너스 (질문/강조 부호는 실제 신호라 유지하되 약간 축소)
  if (t.endsWith('?')) { score += 6; if (category !== 'question') category = 'question'; }
  if (t.endsWith('!')) { score += 4; }
...
function extractPoints(captions, count = 13, seed = null) {
  ...
  // 점수 14 미만은 포인트감 부족 → 후보에서 제외 (count가 높아도 억지로 안 넣음)
  const filtered = scored.filter(s => s.score >= 14);
...
// ============================================
// 추천 개수 추정 - 영상 내용 밀도 + 길이 기반
// ============================================
function estimatePointCount(captions) {
  if (!captions || captions.length === 0) return 10;

  const merged = mergeShortCaptions(captions);
  const scored = merged.map((c, i) => {
    const { score } = scoreText(c.text, i, merged.length);
    return score;
  });

  // 점수대별 후보 개수 (보수적으로 - 진짜 강한 것만 셈)
  const veryStrong = scored.filter(s => s >= 25).length;  // 확실한 포인트
  const strong = scored.filter(s => s >= 18 && s < 25).length; // 강한 후보

  // 영상 길이 (분)
  const totalSec = merged.length > 0 ? merged[merged.length - 1].end : 0;
  const minutes = totalSec / 60;

  // 내용 기반 추정: 확실한 거 전부 + 강한 거 70%만 (필러 배제)
  const contentBased = veryStrong + Math.round(strong * 0.7);

  // 길이 기반 상한 (분당 ~3.5개 - 너무 빽빽하지 않게 보수적)
  const lengthCap = Math.max(8, Math.round(minutes * 3.5));

  // 둘 중 작은 값, 합리적 범위로 클램프 (최소 8, 최대 30)
  let suggested = Math.min(contentBased, lengthCap);
  suggested = Math.max(8, Math.min(30, suggested));

  return suggested;
}

// 라이트 정크 필터 - 명백한 비-포인트(인사/추임새/초단문)만 제거
...
module.exports = { extractPoints, scoreText, estimatePointCount, mergeShortCaptions, detectOrdinal, isJunk, CATEGORY_KEYWORDS };
