...
// 임시(브리지) 디렉토리 보장 — 없으면 자동 생성.
// require 시점에 실패하더라도 앱 전체가 죽지 않도록 반드시 try/catch로 감쌈.
function ensureBridgeDir() {
  try {
    ...
    console.error('[premiere-bridge] 브리지 디렉토리 생성 실패:', e.message);
    return false;
  }
}
ensureBridgeDir();
...
`;
    ...
    ensureBridgeDir(); // temp가 중간에 비워졌을 수 있으니 매 호출 시 재확인
 ...
 */
async function importAndCreateCaption(srtPath, beforeIndex = -1) {
  return await executeScript(`
    var srtPath = ${JSON.stringify(srtPath)};
    var beforeIdx = ${parseInt(beforeIndex)};
    var debug = [];
    ...
    // 4. 캡션 트랙 생성 - beforeIdx 정확히 적용
    // 트랙 개수 사전 측정
    var beforeCount = 0;
    try { beforeCount = seq.captionTracks ? seq.captionTracks.numTracks : 0; } catch(e) {}
    ...
    try {
      seq.createCaptionTrack(srtItem, 0, beforeIdx, false);
      trackCreated = true;
      debug.push("방식1 시도");
    } catch(e1) {
      lastErr = e1.message;
      // 시도 2: createCaptionTrack(item, captionFormat, beforeIndex)
      try {
        seq.createCaptionTrack(srtItem, 0, beforeIdx);
        trackCreated = true;
        debug.push("방식2 시도");
      } catch(e2) {
        lastErr = e2.message;
        // 시도 3: 그냥 추가 (append 동작)
        try {
          seq.createCaptionTrack(srtItem, 0);
          trackCreated = true;
          debug.push("방식3 시도 (append fallback)");
        } catch(e3) {
          lastErr = e3.message;
        }
      }
    ...
    // 5. 트랙이 원하는 위치에 안 들어갔으면 QE API로 이동 시도
    var afterCount = beforeCount;
    try { afterCount = seq.captionTracks ? seq.captionTracks.numTracks : afterCount; } catch(e) {}
    debug.push("트랙수 " + beforeCount + " -> " + afterCount);

    // beforeIdx가 -1(맨 뒤)이 아니고, 실제로는 맨 뒤에 들어간 경우 위치 정보 기록
    var newTrackIdx = afterCount - 1;
    if (beforeIdx >= 0 && newTrackIdx !== beforeIdx) {
      debug.push("주의: 요청 위치=" + beforeIdx + ", 실제 위치=" + newTrackIdx + " (PP API 제한)");
    }

    return __result({imported: true, path: srtPath, newTrackIndex: newTrackIdx, requestedIndex: beforeIdx, debug: debug});
  `);
}
