...
### 암묵적 번호 나열 (list) - "첫번째" 없어도
화자가 명시적 번호 없이 병렬 항목을 연달아 말하면, 흐름상 1./2./3. 매기기
- 원본: "어떤 사람을 모을 거고"
  자막: "1. 어떤 사람을 모을 것인가"
- 원본: "또 그 사람들한테 어떤 메시지를 전달할 것인지"
  자막: "2. 그 사람들한테 어떤 메시지를 전달할 것인가"
- 원본: "어떤 상품과 서비스를 준비할 건지가 훨씬 더 중요합니다"
  자막: "3. 어떤 상품과 서비스를 준비할 것인가"
- 원본: "그런 편집자들을 관리하고"
  자막: "1. 편집자들 관리"
- 원본: "또 그런 콘텐츠들이 효율적으로 또 꾸준히 생산될 수 있는 그런 프로세스를 갖고 움직이고 점검하는 것"
  자막: "2. 콘텐츠들이 효율적으로 꾸준히 생산될 수 있는 프로세스를 갖고 점검"
...
// 선별 결과 파싱: "세그먼트번호| 자막" → [{segIndex, text}]
function parseSelectionResult(result) {
  ...
  for (const l of lines) {
    ...
    if (m) {
      ...
      if (text) picks.push({ segIndex, text });
    }
  }
  return picks;
}
  ...
  try {
    ...
    for (const pick of picks) {
      ...
      points.push({
        ...
        category: (ord && ord > 0) ? 'list' : 'emphasis',
