...
5. **태그에 "번호=N"이 있으면 반드시 "N. ~" 형식으로 출력**. 단, 시리즈 중간 항목은 명사형(번호 생략)도 OK. 예: 번호=2 → "2. 두통" 또는 "두통과 어지러움"
...
### 번호 시리즈 자유도 (list)
"첫 번째/두 번째/세 번째"는 "1./2./3." 명사형이 기본이지만,
중간 항목이 자연스러우면 번호 생략하고 명사형만 OK
- 원본: "첫 번째는 안경을 처음 쓰는 경우예요"
  자막: "1. 안경을 처음 사용하는 경우"
- 원본: "자 우리 두 번째는 근거리 작업이 많은 경우예요"
  자막: "근거리 작업이 많은 경우"  (번호 생략도 OK)
  ...
  if ((config.provider === 'gemini' || config.provider === 'claude') && config.apiKey) {
    try {
      ...
      return points.map((p, i) => {
        ...
        // ordinal 안전망 완화: AI가 번호 생략한 경우 그대로 두기 (시리즈 중간 항목 OK)
        // 단 다른 번호가 잘못 붙어있으면 교정
        if (p.ordinal && p.ordinal > 0) {
          const wrongOrdMatch = text.match(/^\s*(\d+)[.)]\s*/);
          if (wrongOrdMatch && parseInt(wrongOrdMatch[1], 10) !== p.ordinal) {
            // 다른 번호 → 올바른 번호로 교체
            text = text.replace(/^\s*\d+[.)]\s*/, `${p.ordinal}. `);
          }
        }
        return { ...p, text };
      });
